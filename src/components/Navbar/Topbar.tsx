            ...
            <div className='flex flex-row justify-center items-center md:gap-4'>
                ...
                <div className='flex flex-row gap-2 items-center justify-center'>
                    <div className='hidden md:flex flex-col items-start justify-center  '>
                        <span className={`text-sm text-[#161D16] font-bold ${geist.className}`}>
                            {userProfile?.name || "Name"}
                        </span>
                        <span className={`text-xs text-[#3D4A3D] font-normal ${inter.className}`}>
                            {userProfile?.email}
                        </span>
                    </div>
                    <div>
                        <div className='w-10 h-10 border-2 border-[#22C55E] rounded-full'>
                            <Image
                                src={userProfile?.profileImg|| "/Topbar/ProfileImage.jpg"}
